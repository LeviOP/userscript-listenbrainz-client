...
window.addEventListener("yt-player-updated", (event) => {
    ...
    if (!initialPlayerResponse) initialPlayerResponse = true;
...
window.addEventListener("yt-page-data-updated", (e) => {
    ...
    // If the reason for the update was the page type changing, don't add
    // settings again. If the player was updated while the page type changed,
    // we can update.
    if (pageTypeChanged) {
        if (playerUpdated) {
            playerUpdated = false;
