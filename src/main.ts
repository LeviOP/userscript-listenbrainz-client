...
import { PanelIdentifier, YtInitialData } from "./YtInitialData";
...
async function trackListening(player: Player) {
    ...
    let previousTime = 0;
    // The amount of the time that the current video haso been played for.
    let timeListened = 0;
...
function getYtInitialData(document: Document): YtInitialData | null {
    ...
    try {
        const ytInitialData: YtInitialData = JSON.parse(matches[1]);
...
function getDescription(ytInitialData: YtInitialData): string | null {
    if (ytInitialData.engagementPanels === undefined) return null;
...
function getTrackPlaylistId(ytInitialData: YtInitialData): string | null {
    if (ytInitialData.engagementPanels === undefined) return null;
    const descriptionPanel = ytInitialData.engagementPanels.find((panel) => panel.engagementPanelSectionListRenderer.panelIdentifier === PanelIdentifier.EngagementPanelStructuredDescription);
    if (descriptionPanel === undefined) return null;
...
function getTracks(ytInitialData: YtInitialData): string[] | null {
...
async function findYouTubeMusicPlaylist(player: Player): Promise<[string, number, string] | null> {
    ...
    if (videoPage === null) return null;

    const videoYtInitialData = getYtInitialData(videoPage);
    if (videoYtInitialData === null) return null;
...
//function waitForElm<E extends Element = Element>(selector: string): Promise<E> {
//    return new Promise((resolve) => {
//        if (document.querySelector(selector)) {
//            return resolve(document.querySelector<E>(selector) as E); //Compiler doesn't recognize that it can't be null (we just checked)
//        }
//
//        const observer = new MutationObserver(() => {
//            if (document.querySelector(selector)) {
//                observer.disconnect();
//                resolve(document.querySelector<E>(selector) as E);
//            }
//        });
//
